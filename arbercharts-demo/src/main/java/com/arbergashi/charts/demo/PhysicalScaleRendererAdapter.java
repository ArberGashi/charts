...
final class PhysicalScaleRendererAdapter extends BaseRenderer {
    ...
    protected void drawData(ArberCanvas canvas, ChartModel model, PlotContext context) {
        if (!ChartAssets.getBoolean("Chart.scale.physical.enabled", true)) {
            return;
        }
        ...
        double margin = ChartScale.scale(12.0);
        double x0 = bounds.x() + margin;
        double y0 = bounds.maxY() - margin;
        double x1 = x0 + length;

        ChartTheme theme = getResolvedTheme(context);
        ArberColor color = theme != null ? theme.getAxisLabelColor() : ColorRegistry.of(160, 160, 160, 255);
        ...
        float tick = ChartScale.scale(4.0f);
