...
public class ChartUiExtendedTest {
    ...
    public void testDpiScalingEffect() {
        ...
        try {
            ChartScale.applyScale(1.0f, () -> panel.paint(g2_0));
        } finally {
            g2_0.dispose();
        }
        double x1 = panel.getDebugContext().getPlotBounds().getX();
        ...
        try {
            ChartScale.applyScale(2.0f, () -> panel.paint(g2));
        } finally {
            g2.dispose();
        }

        double x2 = panel.getDebugContext().getPlotBounds().getX();
    ...
    public void testPanelLayoutChange() {
        ...
        panel.paint(g2);
        g2.dispose();
        double w1 = panel.getDebugContext().getPlotBounds().getWidth();
        double h1 = panel.getDebugContext().getPlotBounds().getHeight();
        ...
        panel.paint(g2);
        g2.dispose();
        double w2 = panel.getDebugContext().getPlotBounds().getWidth();
        double h2 = panel.getDebugContext().getPlotBounds().getHeight();
    ...
    public void testLegendOverlaySmoke() {
        ...
        if (canvas instanceof Container container) {
            for (Component c : container.getComponents()) {
                if (c instanceof com.arbergashi.charts.render.legend.InteractiveLegendOverlay) {
                    legend = c;
                    break;
                }
            }
