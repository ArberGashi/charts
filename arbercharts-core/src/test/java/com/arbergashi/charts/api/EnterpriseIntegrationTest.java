...
public class EnterpriseIntegrationTest {
    ...
    public void testEnterpriseRenderersSmoke() {
        ...
        for (int i = 0; i < 60; i++) {
            ...
            model.addPoint(i, 10 + Math.sin(i * 0.2) * 5, 5, 20, 100, null);
        }

        ArberChartPanel panel = new ArberChartPanel(model, new IchimokuCloudRenderer());
        panel.withTheme(TestThemes.highContrast());
        panel.addLayer(model, new FibonacciRetracementRenderer());
        panel.addLayer(model, new VolumeProfileRenderer());
        panel.addLayer(model, new ECGRhythmRenderer());
    ...
    public void testHighContrastTheme() {
        ...
        assertEquals(Color.BLACK, hc.getBackground());
        assertEquals(Color.WHITE, hc.getForeground());
        assertEquals(Color.YELLOW, hc.getAccentColor());
    }
    ...
    public void testAccessibilityIntegration() {
        ...
        panel.withTheme(ChartThemes.defaultDark());
