...
public final class CandlestickRenderer extends BaseRenderer {
    ...
    protected void drawData(Graphics2D g, ChartModel model, PlotContext context) {
        ...
        final double barWidth = (context.plotBounds().getWidth() / (double) count) * 0.75;
        ...
        for (int i = 0; i < count; i++) {
            ...
            float x = (float) (candleX - barWidth / 2.0);

            // Clipping
            double wickTop = Math.min(highY, lowY);
            double wickHeight = Math.abs(lowY - highY);
            if (!viewBounds.intersects(x, wickTop, barWidth, wickHeight)) {
                continue;
            }
            ...
            g.draw(getLine(candleX, highY, candleX, lowY));

            // 2. Body
            Shape body = getRoundRectangle(x, (float) bodyY, (float) barWidth, (float) bodyH,
